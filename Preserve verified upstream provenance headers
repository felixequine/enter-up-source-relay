...
export default {
  async fetch(request, env, ctx) {
    ...
    // The ingestion worker verifies this exact header before accepting relayed
    // bytes as authoritative source material. Keep it stable for compatibility.
    headers.set("x-enter-up-official-source", source.url);
    headers.set("x-enter-up-fetched-at", new Date().toISOString());
